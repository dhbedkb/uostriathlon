...
*(Superseded by Phase 3, below — content_order and visibility as
separate fields no longer exist. This section is kept as a record of
what Phase 2 was and why, per this file's own established convention
of keeping prior phases rather than rewriting history.)*

## 10. Phase 3 addendum: content blocks replace content_order + visibility

Phase 2's two-field system (`content_order` list, `visibility` object)
worked, but asked an editor to keep two things in sync — "I added Body
to the order, now I need to remember to also set its visibility." Phase
3 collapses that into one thing: a tile's `blocks` list **is** its
content, in render order, full stop. There is no field anywhere else
that also has to agree with it.

**The data model**, per tile:

```yaml
badge: "April"          # unchanged — tile-level, not a block, no reveal state
style: default           # unchanged — tile-level visual variant
blocks:
  - {type: title, text: "Barnaby Read"}
  - {type: subtitle, text: "Secretary"}
  - {type: image, src_raw: "...", crop_x: 50, crop_y: 50, crop_zoom: 100}
  - {type: expand}
  - {type: body, text: "Runs committee meetings..."}
  - {type: email, value: "triathlon@sheffield.ac.uk"}
```

- **Ordering** is native Decap `list` widget drag-and-drop — no
  separate ordering field, because the thing being dragged is the
  content itself.
- **Visibility** is a single marker block, `type: expand`. Everything
  before the first one renders always; everything from it onward
  renders inside the hover/click reveal region. No expand block → no
  reveal region at all, tile renders flat. This replaces `hidden`,
  `always`, `hover`, and `click` as four separate per-field states with
  one position in one list — "hidden" specifically is gone entirely,
  because a block simply not being in the list already means hidden.
  There's no longer a way to have two things both true (in the list,
  but marked hidden) — a needless second way to express the same idea.
- **Hover vs click** stopped being resolvable per-tile (Phase 2 let a
  tile's own field visibility choices infer its trigger). Since Phase 3
  no longer has per-field visibility to infer *from*, this reverted to
  a single site-wide setting, `site.data.settings.interactions.expand_trigger`
  — the same mechanism this codebase had prior to Phase 2, brought back
  because the reason Phase 2 needed a per-tile inferred trigger (mixed
  hover/click states across a tile's fields) no longer exists as a
  concept.
- **Meta rows and Q&A pairs merged into one block type, `note`**
  (`heading` + `detail`). They were always the same shape rendered two
  different ways for no functional reason; Phase 3 keeps one rendering
  (bold heading, plain text beneath) and drops the compact inline
  "**Label** value" style that `meta` used to have.

**Presets became genuinely functional**, using Decap's typed-list
mechanism — the same one `Sections` already uses to offer
Hero/Text/Divider bar/Card grid as distinct choices. `Tiles` is now a
typed list (`typeKey: preset`) offering five choices when you click
"Add Tile": Committee member, FAQ item, Sponsor, Event, and Blank tile.
Each of the first four ships a `default: blocks: [...]` matching what
that kind of content usually needs; Blank tile ships `default: blocks:
[]`. This is real, tested Decap functionality — not a custom widget —
so it carries none of the risk a bespoke insert-from-preset control
would.

**The one thing this doesn't do, on purpose:** presets aren't editable
from inside the CMS at runtime. An earlier plan considered a custom
"insert tile from a preset stored in Site Settings" control, editable
live by content editors with no code change. Building that safely would
require either relying on undocumented Decap internals to read another
collection's data from inside a field control (unverifiable without a
live browser and a running Decap instance, neither of which were
available while building this), or reimplementing Decap's `list` widget
and its GitHub-backed image upload UI from scratch just to add one
button — a large, genuinely risky rewrite of infrastructure that
currently works, for a feature (runtime-editable presets) nobody has
confirmed is actually needed day to day. The typed-list version gets
every concrete requirement that was actually specified (blank tile on
insert, pre-filled tile on preset insert, no preset "hint" surviving
into the rendered page, native drag reorder) using a fully-supported
mechanism, at the cost of "changing what a preset contains" being a
`config.yml` edit instead of a CMS click. That trade-off is stated
explicitly rather than silently substituted — see "Presets" in
`docs/editor-guide.md`.

**File-by-file summary of what changed:** `_includes/tile.html` (full
rewrite: one loop over `tile.blocks`, splitting on first `expand`
instead of resolving `content_order`/`visibility`), `_includes/tile-chip.html`
and `_includes/tile-grid.html` (updated to read from `tile.blocks`,
and no longer pass or read a `behavior`/`expand_default` value),
`admin/config.yml` / `admin/config.local.yml` (Tiles becomes a typed
list of five presets, each with a `blocks` typed sub-list; Sections'
`preset` and `behavior` fields on Card grid removed entirely, since
preset now lives per-tile and expand lives per-block),
`admin/crop-preview.js` (mirrors the same block-splitting logic),
`scripts/optimise_site_images.py` (image lookup changed from
`tile.image` to scanning `tile.blocks` for any block of `type: image`,
of which there can now be more than one per tile), and
`scripts/migrate_blocks.py` (new one-off migration, applied once to all
six `_data/content/*.yml` files; kept in the repo as a record, same
convention as `scripts/migrate_content.py`).
